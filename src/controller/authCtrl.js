...
const authCtrl = {
  ...
  signup: async (req, res) => {
    try {
      ...
      if (!firstname || !lastname || !email || !req.body.password) {
        return res.status(403).json({ message: "Place fill all fields" })
      }
      ...
      const token = JWT.sign(other, JWT_SECRET_KEY)
  ...
  login: async (req, res) => {
    try {
      ...
      const token = JWT.sign(user._doc, process.env.JWT_SECRET_KEY, { expiresIn: "24h" });
